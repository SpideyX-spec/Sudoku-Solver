...
const BASE_SOLVED = {
  ...
  16: Array.from({ length: 16 }, (_, r) =>
    Array.from({ length: 16 }, (_, c) => SYMBOLS[16][(c + (r * 4 + Math.floor(r / 4)) % 16) % 16])
  )
};
...
const message = (txt) => $('message').textContent = txt;
...
function startManualTimer(resume = false) {
  ...
  if (!resume || !state.remainingSeconds) state.remainingSeconds = currentTimerSeconds();
...
function permuteSolved(size) {
  ...
  for (let r = 0; r < size; r++) {
    for (let c = 0; c < size; c++) solved[r][c] = map.get(solved[r][c]);
  }
  return solved;
}
...
async function solveWithTime(animated = false) {
  ...
  saveScore(username, Math.round(100000 / (parseFloat(elapsed) + 1)), state.difficulty, state.size, region, elapsedSec);
}
...
async function loadLeaderboard() {
  try {
    ...
    $('leaderboardList').innerHTML = data.map(
      x => `<li>${x.username} — ${x.score} pts • ${formatDuration(x.time_seconds)} (${x.region || 'Global'})</li>`
    ).join('');
    const body = $('leaderboardTableBody');
    if (body) body.innerHTML = data.map(x => `<tr><td>${x.username}</td><td>${x.score}</td><td>${x.region || 'Global'}</td><td>${formatDuration(x.time_seconds)}</td></tr>`).join('');
  } catch {
...
function bindSidebar() {
  ...
  $('leaderboardIcon').addEventListener('click', () => $('leaderboardPanel').classList.toggle('hidden'));
}
...
function bindUI() {
  ...
  $('resumeBtn').addEventListener('click', () => { if (!resumeProgress()) message('No saved game found.'); });
  $('restartBtn').addEventListener('click', () => { localStorage.removeItem('sudoku_progress'); startNewGame(); showPage('gamePage'); });
